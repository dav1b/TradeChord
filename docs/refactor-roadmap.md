...
- The primary country projection still publishes partner×product detail only for
  `crossYear` (the release headline year). The historical partner×product×year
  detail is now published in an **additive per-country detail sidecar**
  (`detail/<CODE>.json`) generated by `reproject` into release `2026-07.2`. See
  [detail-sidecar.md](./detail-sidecar.md). The client must still not infer those
  values from national product totals — it reads the sidecar or shows an explicit
  unavailable state. As of this writing the sidecar exists but no frontend journey
  consumes it yet, and `current.json` is deliberately kept on `2026-07.1` until it
  does.
