...
<style>
	.panel {
		position:absolute; z-index:5; top:51%; left:50%; width:min(88%,1180px);
	...
	.overview { display:grid; grid-template-columns:minmax(300px,.9fr) minmax(420px,1.4fr);
		align-items:center; gap:clamp(24px,5vw,72px); margin-bottom:22px; }
	...
	.measure:last-of-type { border-right:1px solid var(--border); }
	...
	.metrics p { grid-column:1/-1; margin-top:7px; text-transform:none; }
	@media(max-width:760px) {
		.panel { top:106%; width:calc(100% - 20px); max-height:82svh; padding:16px 14px; }
